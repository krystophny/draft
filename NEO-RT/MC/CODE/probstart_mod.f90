...
CONTAINS
!  INCLUDE 'cx_ion.f90'
  INCLUDE 'ionization_crossec.f90'
  !
  ...
  SUBROUTINE calc_probstart(lvec, es_lvec, plasma_mat, E_beam, amb, probstart_vec)
    ...
    ! local definitions
    INTEGER :: ind, num_prof, num_l
    REAL(kind=dp) :: v0, delta_l, nu_ov_v0, sigma_cx, Ebeam_eff,Ti_eff,sigmav_elimp,sigmav_ioimp
...
!!$    END DO
    ...
    DO ind = 1,num_l
!
       call sigmav_ionisation_by_e(ni_prof(ind),Te_prof(ind),sigmav_elimp)
!
       Ebeam_eff=E_beam/amb
       Ti_eff=Ti_prof(ind)/amb
!
       call sigmav_ionisation_by_i(Ebeam_eff,Ti_eff,sigmav_ioimp)
!
       nui_ov_v0(ind) = (sigmav_elimp+sigmav_ioimp)*ni_prof(ind)/v0
!write(1001,*) dble(ind),sigmav_ion(Te_prof(ind)),sigmav_elimp,sigmav_ioimp
